...
class Litmas extends CI_Controller {
	private $data;
	...
	public function lists(){
		...
		if(isset($_POST['submit'])){
			if (isset($_POST['nama'])) {
				if ($_POST['nama'] != '' or $_POST['nama'] != null) {
					$filters[] = "NAMA_LITMAS LIKE '%" . $_POST['nama'] . "%'";
					$r_nama = $_POST['nama'];
				}
			}			
		...
		$data = $this->litmas_model->get($filters, $limit);
		//var_dump($data);
		$total_data = count($this->litmas_model->get($filters));
		$limit[] = $total_data;
		...
		$header = array(
			array (
				...
				(object) array ('colspan' => 1, 'classes' => 'bold align-center capitalize', 'value' => 'nama litmas'),								
				(object) array ('rowspan' => 1, 'classes' => 'bold align-center capitalize', 'value' => 'status'),			
			)		
		);

		$status = array();
		$status[] = (object) array('label'=>'1 | Aktif', 'value'=>'1');
		$status[] = (object) array('label'=>'0 | Tdk Aktif', 'value'=>'0');
			
		$fields = array();
		$fields[] = (object) array(
			'type' 			=> 'text',
			'label' 		=> 'Nama Litmas',
			'placeholder' 	=> 'Nama Litmas',
			'name' 			=> 'nama',
			'value' 		=> $r_nama,
			'classes' 		=> 'full-width',
		);		
	...
	public function insert(){
		...
		if(isset($_POST['submit'])){
			...
			$error_status = false;
			if($_POST['nama'] == ''){
				$error_info[] = 'Nama Litmas tidak boleh kosong';
				$error_status = true;
			}
			...
			if($error_status == true){
				...
				$this->data['insert'] = array(
					'NAMA_LITMAS' => $_POST['nama'],
				);	
				//var_dump($this->data['insert']);die;
				$result = $this->litmas_model->insert($this->data['insert']);
				$info = array();
	...
	public function update(){
		...
		if(isset($_POST['submit'])){
			...
			$filter = array();
			$filter[] = "ID = ". $_POST['id'];
			$this->data['result'] = $this->litmas_model->get($filter);
			foreach($this->data['result'] as $value){
				$r_id 	= $value->ID;
				$r_nama = $value->NAMA_LITMAS;
			}
	...
	public function detail($id=null){
		...
		if(isset($_POST['id']) and $_POST['id'] != null){
			$filters = array();
			$filters[] = "ID = ". $_POST['id'];
			$data = $this->litmas_model->get($filters);
 	...
 	public function update_status(){
		...
		if(isset($_POST['id']) and $_POST['id'] != null){
			...
			$result = $this->litmas_model->get($filters);
			if($result != null){
...
/* 	public function delete(){
		...
		$result = $this->litmas_model->delete($this->data['delete']);
